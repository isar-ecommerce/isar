...
const DEFAULT_BANNERS: HeroBannerItem[] = [
  {
    ...
    linkUrl: '/products',
    bgGradient: 'from-black via-[#090d16] to-[#0f172a]',
  },
  {
    id: 'banner-2',
...
export default function Home() {
  ...
  const renderProductCard = (product: Product) => {
    ...
    return (
      <div 
        key={product.id} 
        className="w-44 sm:w-52 md:w-60 bg-white dark:bg-[#0f141c] rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all group border border-gray-200/80 dark:border-[#1e2638] flex flex-col shrink-0"
      >
        <Link to={`/products/${product.slug || product.id}`} className="relative aspect-square overflow-hidden bg-gray-50/60 dark:bg-[#141b26] p-2.5 flex items-center justify-center">
          {isOutOfStock && (
  ...
  return (
    <div className="w-full bg-secondary min-h-screen pb-12 space-y-4 sm:space-y-6">
      ...
      <section className="container mx-auto px-3 sm:px-4">
        <div className="bg-white dark:bg-[#0f141c] rounded-3xl p-3 sm:p-4 shadow-sm border border-gray-200/80 dark:border-[#1e2638]">
          <div className="flex sm:grid sm:grid-cols-4 gap-3 sm:gap-6 overflow-x-auto no-scrollbar">
            <div className="flex items-center gap-2.5 shrink-0 px-3 py-1.5 rounded-xl bg-gray-50/80 dark:bg-[#141b26] sm:bg-transparent">
            ...
            {categories.map((category) => {
              ...
              return (
                <Link 
                  ...
                  className="bg-white dark:bg-[#0f141c] rounded-2xl p-3 flex flex-col items-center justify-center text-center gap-1.5 shadow-sm hover:shadow-md transition-all border border-gray-200/80 dark:border-[#1e2638] group w-28 sm:w-36 shrink-0 min-h-24 cursor-pointer hover:border-primary/50"
                >
